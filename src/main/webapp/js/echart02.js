...
//                        links:relationship
	...
	$.ajax({
        ...
        error : function(XMLHttpRequest, textStatus, errorThrown) {
       	 $("#p_test").innerHTML = "there is something wrong!";
          alert(XMLHttpRequest.status);//ajax错误报告
          alert(XMLHttpRequest.readyState);
          alert(textStatus);
        }
   })
...
//	                        links:relationship
	 ...
	 function appendPath(params){    //拼接节点关系并显示在左下角，
         ...
         if(params.dataType=="node"){ //dataType可以是edge(线条)或node(数据)
             ...
             var str="姓名:"+name+'<br/>'+"简介："+info+'<br/>'+
       	  "日期："+date+'<br/>'+"电话："+tel+'<br/>'+"备注："+remark;
             if(category==1){
            	 var str = "姓名:"+params.data.name+'<br/>'+
            	 "家庭住址:"+params.data.jiatingzhuzhi+'<br/>'+
            	 "联系电话:"+params.data.lianxidianhua+'<br/>'+
            	 "相关简介:"+params.data.xiangguanjianjie+'<br/>'+
            	 "燃料类型:"+params.data.ranliaoleixing+'<br/>'+
            	 "家庭人数:"+params.data.jiatingrenshu+'<br/>'+
            	 "受捐金额:"+params.data.shoujuanjine_yuan+'<br/>'+
            	 "贫困原因:"+params.data.pinkunyuanyin;
            	 return str;
             }
             if(category==2){
            	 var str = "姓名:"+params.data.name+'<br/>'+
             ...
             if(category==3){
            	 var str = "姓名:"+params.data.name+'<br/>'+
            	 "与户主关系:"+params.data.yuhuzhuguanxi+'<br/>'+
            	 "务工时间:"+params.data.wugongshijian+'<br/>'+
            	 "文化程度:"+params.data.wenhuachengdu+'<br/>'+
            	 "劳动能力:"+params.data.laodongnengli;
            	 return str;
             }
             if(category==4){
